...
D:/Python/pythonw.exe -u dariel/qq_alarm.py &          # 动态闹钟(后台盯消息)
```
**禁止批量杀Python进程。** 桥接PID在 `dariel/tts/qq_bridge.pid`。
**后台进程统一用 `pythonw.exe`，无终端弹窗。**

### 第5步: QQ 消息 (动态闹钟)
QQ 消息由 `qq_alarm.py` + `[qq-wake]` 动态闹钟处理，不再轮询。
...
`qq_watch.py` 作为 `run_in_background` 任务运行。死等 `qq_push.json`，消息来就写 trigger 后退出。
退出时 Claude Code 自动注入 `<task-notification>`。

**收到任意 `<task-notification>`（后台任务完成）时：**
- 读 `dariel/qq_alarm_trigger.json`
- 若不存在 → 忽略，什么都不做
- 若存在 → 读 `dariel/alarm_last.json`
  - 若 `alarm_at` == `last_alarm_at` → 已处理，删 trigger，重启 `qq_watch.py &`
  - 若为新闹钟：
    1. 读 `dariel/tts/conv_state.json`
    2. 调 `mcp__dariel-unified__qq` (action=full)
    3. 逐条回复思思(3165473685)
    4. 记 `alarm_at` 到 `dariel/alarm_last.json`
    5. 删 `qq_alarm_trigger.json` 和 `poll_wake.flag`
    6. 重启后台守望：`pythonw dariel/qq_watch.py &`

**BP1 启动时必须拉起守望：** `pythonw dariel/qq_watch.py &`

**无消息时：** `qq_watch.py` 在后台静默运行，不消耗 Claude Code token。
...
### 定时任务 (cron)
```
# keepalive自主唤醒 — 每55分钟 (pythonw无窗口)
*/55 * * * * cd /c/Users/31654/Desktop && D:/Python/pythonw.exe dariel/keepalive.py
```
