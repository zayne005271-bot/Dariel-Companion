...
```bash
# ── 基础服务 (全部独立 pythonw 进程，换窗不丢) ──
docker start napcat                                          # NapCat (如未运行)
D:/Python/pythonw.exe -u dariel/restart_bridge.py            # 安全重启桥接(只杀桥接PID)
D:/Python/pythonw.exe -u dariel/qq_watch.py &                # ① QQ守望(独立进程)
D:/Python/pythonw.exe -u dariel/session_watcher.py &         # ② 切窗守护(独立进程)
D:/Python/pythonw.exe -u dariel/dream_events.py &            # iOS感知层(如未监听8765)

# ── 守望四件套 (全部 run_in_background) ──
python C:/Users/31654/Desktop/dariel/trigger_alarm.py       # ① trigger闹钟(10s查trigger，发现退出通知CC)
...
**守望启动：**
`D:/Python/pythonw.exe -u dariel/qq_watch.py &`    ← 独立进程(换窗不丢)
